...
class PartnersetPage(object):
	...
	def submit(self, btn_name):
		try:
			...
			print('[PartnerSetPage]submit－－点击' + btn_name + ' 失败－－失败原因是＝>', str(e))
			exit()
	...
	def add_partnerset(self, partnerset_info):
		...
		if not publicPage.wait_until_loader_disapeared():
			...
			print('[PartnersetPage]add_partnerset－－去股东页面失败－－当前页面url＝>', self.driver.current_url)
	...
	def click_edit_btn(self, name):
		...
		if name_locs.text == name:
			edit_elem = names_elem + '../following-sibling::td/button[@id="editDepartmentInfo"]'
	...
	def eidt_partnerset(self, edit_partnerset_info):
		...
		if not publicPage.wait_until_loader_disapeared():
			if '/app/setting/partner-set' in self.driver.current_url:
				...
				time.sleep(1)
				self.set_partnerset_name(edit_partnerset_info[1] + str(random_num))
			else:
	...
	def get_names(self, name_td_index):
		...
		for i in self.get_row_indexs():
			if i == 0:
				i = i + 1
			name_loc = self.driver.find_elements_by_tag_name('tr')[i].find_elements_by_tag_name('td')[
	...
	def click_operation_btn(self, name_td_index, item_name, btn_td_index, btn_name):
		...
		if btn_name == 'edit':
			...
			btn_index = 1
		btn_loc = self.driver.find_elements_by_tag_name('tr')[tr_index].find_elements_by_tag_name('td')[
